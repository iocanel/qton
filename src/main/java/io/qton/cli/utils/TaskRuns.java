...
import static io.qton.cli.utils.Pods.isCompleted;
import static io.qton.cli.utils.Pods.waitIfPending;
  ...
  public static void log(String name) {
    Clients.kubernetes().pods().withLabel(TASKRUN_LABEL, name).list().getItems().forEach(p -> {
      ...
      p.getSpec().getContainers().forEach(container -> {
        try (LogWatch watch = Clients.kubernetes().pods().withName(p.getMetadata().getName()).inContainer(container.getName())
